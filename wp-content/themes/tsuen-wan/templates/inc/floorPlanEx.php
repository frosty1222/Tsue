...
<style>
    <?php if ($is_no_controls) { ?>.controls .floors,
    .controls.facilities {
        display: none;
    }
...
<script>
    ...
    for (var shopNumber in shopData) {
        ...
        var shopListShop = $('<a>').attr('href', 'javascript:void(0)').data('shop', shopNumber).addClass('shop').click(function() {
            focusShop($(this).data('shop'));
        });
        var shopVerticalCenterer = $('<div>').addClass('shop-vertical-centerer');
        if (shopRow.featured) {
            shopVerticalCenterer.append($('<img>').attr('src', shopRow.featured));
        } else {
            shopVerticalCenterer.append($('<img>').attr('src', shopRow.thumbnail));
        }
        shopVerticalCenterer.append($('<div>').addClass('shop-info')
            .append($('<div>').addClass('shop-name').text(shopRow.name))
            .append($('<div>').addClass('shop-floor-number').text(shopRow.shop_location_text))
        );
        var shopInfoMobile = '<div class="content-for-mobile" style="display: none">' +
            '<h5>' + shopRow.name + '</h5>' +
            '<div class="shop-name">' +
            '<span class="icon">' +
        ...
        shopInfoMobile += '<div class="phone-number">' +
            ...
            '</div>';
        if (shopRow.privileges) {
            shopInfoMobile += '<ul class="listmenu list-icon" style="display: flex">';
            var privileges_array = Object.keys(shopRow.privileges).map((key) => [key, shopRow.privileges[key]]);
            for (var i = 0; i < privileges_array.length; i++) {
                //console.log(privileges_array[i]);
                shopInfoMobile += '<li><img src="' + privileges_array[i][1] + '"/></li>';
            }
            shopInfoMobile += '</ul>';
        }
        // '<ul class="listmenu list-icon" style="display: flex">'+
        //     '<li>'+
        //         '<svg width="23" height="15" viewBox="0 0 23 15" fill="none" xmlns="http://www.w3.org/2000/svg">'+
        //             '<rect x="1.16953" y="0.4" width="21.2" height="14.2" rx="1.6" stroke="black" stroke-width="0.8"></rect>'+
        //             '<path d="M12.0391 12C10.8371 12 9.70701 11.5319 8.85707 10.682C8.00713 9.83205 7.53906 8.70199 7.53906 7.5C7.53906 6.29801 8.00715 5.16796 8.85707 4.31801C9.70699 3.46805 10.8371 3 12.0391 3C13.2411 3 14.3711 3.46807 15.2211 4.31801C16.071 5.16795 16.5391 6.29801 16.5391 7.5C16.5391 8.70199 16.071 9.83204 15.2211 10.682C14.3711 11.5319 13.2411 12 12.0391 12ZM12.0391 3.5625C9.86792 3.5625 8.10156 5.32886 8.10156 7.5C8.10156 9.67114 9.86792 11.4375 12.0391 11.4375C14.2102 11.4375 15.9766 9.67114 15.9766 7.5C15.9766 5.32886 14.2102 3.5625 12.0391 3.5625Z" fill="black"></path>'+
        //             '<path d="M12.0391 7.21875C11.6514 7.21875 11.3359 6.90333 11.3359 6.51562C11.3359 6.12792 11.6514 5.8125 12.0391 5.8125C12.4268 5.8125 12.7422 6.12792 12.7422 6.51562C12.7422 6.67095 12.8681 6.79688 13.0234 6.79688C13.1788 6.79688 13.3047 6.67095 13.3047 6.51562C13.3047 5.91444 12.8832 5.41017 12.3203 5.28189V4.96875C12.3203 4.81343 12.1944 4.6875 12.0391 4.6875C11.8837 4.6875 11.7578 4.81343 11.7578 4.96875V5.28189C11.1949 5.41017 10.7734 5.91444 10.7734 6.51562C10.7734 7.21349 11.3412 7.78125 12.0391 7.78125C12.4268 7.78125 12.7422 8.09667 12.7422 8.48438C12.7422 8.87208 12.4268 9.1875 12.0391 9.1875C11.6514 9.1875 11.3359 8.87208 11.3359 8.48438C11.3359 8.32905 11.21 8.20312 11.0547 8.20312C10.8993 8.20312 10.7734 8.32905 10.7734 8.48438C10.7734 9.08556 11.1949 9.58983 11.7578 9.71811V10.0312C11.7578 10.1866 11.8837 10.3125 12.0391 10.3125C12.1944 10.3125 12.3203 10.1866 12.3203 10.0312V9.71811C12.8832 9.58983 13.3047 9.08556 13.3047 8.48438C13.3047 7.78651 12.7369 7.21875 12.0391 7.21875Z" fill="black"></path>'+
        //         '</svg>'+
        //     '</li>'+
        // '</ul>';
        shopInfoMobile += '</div>';
